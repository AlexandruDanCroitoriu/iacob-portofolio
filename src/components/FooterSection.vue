<template>
  <footer class="bg-gray-900 text-white py-16">
    <div class="container mx-auto px-4">
      <div class="max-w-6xl mx-auto">
        <div class="grid md:grid-cols-4 gap-8">
          
          <!-- Brand Column -->
          <div class="md:col-span-2">
            <div class="mb-6">
              <h3 class="text-2xl font-bold mb-2">
                {{ iacobData.personal.name.split(' ')[0] }}<span class="text-red-500"> {{ iacobData.personal.name.split(' ')[1] }}</span>
              </h3>
              <p class="text-gray-400 mb-4">{{ iacobData.personal.title }}</p>
              <p class="text-gray-300 italic">"{{ iacobData.footer.tagline }}"</p>
            </div>
            
            <!-- Social Media Links -->
            <div class="flex space-x-4">
              <a 
                :href="iacobData.socialMedia.instagram" 
                class="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-red-500 transition-colors duration-300"
                aria-label="Instagram"
              >
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 6.62 5.367 11.987 11.987 11.987s11.987-5.367 11.987-11.987C24.014 5.367 18.647.001 12.017.001zM8.23 18.678c-2.44 0-4.41-1.97-4.41-4.41V9.732c0-2.44 1.97-4.41 4.41-4.41h7.54c2.44 0 4.41 1.97 4.41 4.41v4.536c0 2.44-1.97 4.41-4.41 4.41H8.23z"/>
                </svg>
              </a>
              <a 
                :href="iacobData.socialMedia.youtube" 
                class="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-red-500 transition-colors duration-300"
                aria-label="YouTube"
              >
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                </svg>
              </a>
              <a 
                :href="iacobData.socialMedia.facebook" 
                class="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-red-500 transition-colors duration-300"
                aria-label="Facebook"
              >
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
              <a 
                :href="iacobData.socialMedia.linkedin" 
                class="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-red-500 transition-colors duration-300"
                aria-label="LinkedIn"
              >
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                </svg>
              </a>
            </div>
          </div>

          <!-- Quick Links Column -->
          <div>
            <h4 class="text-lg font-semibold mb-4">Quick Links</h4>
            <ul class="space-y-2">
              <li v-for="link in iacobData.footer.quickLinks" :key="link.text">
                <button 
                  @click="scrollToSection(link.href)"
                  class="text-gray-400 hover:text-white transition-colors duration-300 text-left"
                >
                  {{ link.text }}
                </button>
              </li>
            </ul>
          </div>

          <!-- Contact Info Column -->
          <div>
            <h4 class="text-lg font-semibold mb-4">Contact Info</h4>
            <div class="space-y-2 text-gray-400">
              <p>{{ iacobData.personal.phone }}</p>
              <p>{{ iacobData.personal.email }}</p>
              <p>{{ iacobData.personal.location }}</p>
            </div>
          </div>
        </div>

        <!-- Divider -->
        <div class="border-t border-gray-800 mt-12 pt-8">
          <div class="flex flex-col md:flex-row justify-between items-center">
            <!-- Copyright -->
            <div class="text-gray-400 mb-4 md:mb-0">
              <p>&copy; {{ currentYear }} {{ iacobData.personal.name }}. All rights reserved.</p>
            </div>

            <!-- Legal Links -->
            <div class="flex space-x-6">
              <a 
                v-for="link in iacobData.footer.legalLinks" 
                :key="link.text"
                :href="link.href" 
                class="text-gray-400 hover:text-white transition-colors duration-300 text-sm"
              >
                {{ link.text }}
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </footer>
</template>

<script setup>
import iacobData from '../data/iacobData.js'
import { computed } from 'vue'

// Get current year for copyright
const currentYear = computed(() => new Date().getFullYear())

// Function to scroll to section smoothly
const scrollToSection = (href) => {
  // Remove the # from href to get the element ID
  const elementId = href.replace('#', '')
  const targetElement = document.getElementById(elementId)
  
  if (targetElement) {
    targetElement.scrollIntoView({
      behavior: 'smooth',
      block: 'start'
    })
  }
}
</script>

<style scoped>
/* Responsive adjustments */
@media (max-width: 768px) {
  .container {
    padding-left: 1rem;
    padding-right: 1rem;
  }
  
  .grid {
    gap: 2rem;
  }

  .flex {
    flex-wrap: wrap;
  }
}
</style>
